...
def compress_img(image_name, new_size_ratio=0.9, quality=90,
                 width=None, height=None, to_jpg=True, to_png=True):
...
if __name__ == '__main__':
  ...
  parser.add_argument('-p', '--to-png', action='store_true',
                      help='Whether to convert the image to the PNG format')
  parser.add_argument('-q', '--quality', type=int,
                      help='Quality ranging from a minimum of 0 (worst) to '
                      'a maximum of 95 (best). Default is 90',
                      default=90)
  parser.add_argument('-r', '--resize-ratio', type=float,
                      help='Resizing ratio from 0 to 1, setting to 0.5 will '
                      'multiply width & height of the image by 0.5. Default '
                      'is 1.0',
                      default=1.0)
  parser.add_argument('-w', '--width', type=int,
                      help='The new width image, make sure to set it with the'
                      ' `height` parameter')
  parser.add_argument('-hh', '--height', type=int,
                      help='The new height for the image, make sure to set it'
                      ' with the `width` parameter')
  args = parser.parse_args()

  if args.to_jpg and args.to_png:
    print('`to_jpg` and `to_png` cannot be'
  ...
  compress_img(args.image, args.resize_ratio, args.quality,
               args.width, args.height, args.to_jpg, args.to_png)
