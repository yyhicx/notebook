...
def process_declaration_block(declaration_block, properties):
    ...
    while first_letter_index != -1:
        ...
        properties_and_values.update(
            {
                declaration_block[first_letter_index:colon_index]: declaration_block[
                    first_letter_index : semicolon_index + 1
                ]
            }
        )
    ...
    sorted_properties_and_values = sorted(
        properties_and_values.items(), key=lambda x: properties.index(x[0])
    )
...
def process_data(input_data, properties):
    ...
    while left_brace_index != -1:
        ...
        new_declaration_block = process_declaration_block(declaration_block, properties)
...
def main(filename):
    ...
    ext_length = len(".css")
    with open(
        filename[:-ext_length] + "-processed" + filename[-ext_length:],
        "w",
        encoding="utf-8",
    ) as f:
        f.write(output_data)
